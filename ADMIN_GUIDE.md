...
- **Hetzner box** (`46.224.28.213`): `/opt/bario-storage/pg-backup.sh`, cron `15 8 * * *`. Originally also covered `unique-crm-stack`/`bario-crm-stack`, but both are now **fully decommissioned** (2026-08-20, confirmed intentional — user stopped them for cost reasons and wants Twenty gone entirely). Real data was confirmed and preserved first: `unique-crm-stack` had 3 real contacts (Sherwin/Mya/Julianna, from Victoria's own call-logging) + 6 real notes, migrated into Bario One's CRM (`bo_customers`/`bo_notes`, org `f7e3dc4b-fb3c-41eb-a24d-6339491c927c`) via a one-off admin route reading Postgres directly (Twenty's own GraphQL had no nginx/cert in front of it anymore); `bario-crm-stack` had zero real data, only Twenty's default demo-seed contacts. Both stacks' `docker-compose.yml`/`pgdata` were then deleted from disk — the pre-deletion tarball backups are the only copies left, in the private `bario-db-backups` MinIO bucket. Victoria's call-logging (`lib/barioOneCrmCallLog.ts`) now points `unique`/`bario` at Bario One directly, same as `afc`/`sunbuilt` already did — verified live with a real test call-log write to each before the Twenty stacks were removed.
