...
export default function PricingSection() {
  ...
  return (
    <>
      <div className="flex items-center justify-center gap-1 mb-10 bg-slate-900 border border-slate-800 rounded-full p-1 w-fit mx-auto">
        <button
          onClick={() => setCycle('monthly')}
          className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${cycle === 'monthly' ? 'bg-cyan-500 text-slate-950' : 'text-slate-400'}`}
        >
          Monthly
        </button>
        <button
          onClick={() => setCycle('annual')}
          className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${cycle === 'annual' ? 'bg-cyan-500 text-slate-950' : 'text-slate-400'}`}
        >
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="text-4xl font-extrabold text-white">
                    ${effectiveMonthly.toFixed(cycle === 'annual' ? 2 : 0)} <span className="text-sm text-slate-400 font-normal">/mo CAD</span>
                  </div>
                  {cycle === 'annual' && (
