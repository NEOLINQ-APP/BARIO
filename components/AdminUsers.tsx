    ...
    <main className="min-h-screen bg-white dark:bg-[#0b111c] text-slate-900 dark:text-zinc-100 antialiased px-6 py-16">
      <div className="max-w-lg mx-auto space-y-10">
        ...
        <div>
          ...
          <form onSubmit={handleGrant} className="mt-6 rounded-2xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-[#131b2a] p-6 space-y-4">
            <div>
              <label className="block text-xs text-slate-500 dark:text-zinc-400 mb-1">Account email</label>
              <input
                type="email"
                value={email}
            ...
            <div>
              <label className="block text-xs text-slate-500 dark:text-zinc-400 mb-1">Plan</label>
              <select
                value={plan}
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1">
            Comp an X-Drive storage tier — separate from the site plan above. Same deal: no payment, no Stripe subscription, stays active until changed here.
          </p>

          <form onSubmit={handleGrantStorage} className="mt-6 rounded-2xl border border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-[#131b2a] p-6 space-y-4">
            <div>
              <label className="block text-xs text-slate-500 dark:text-zinc-400 mb-1">Account email</label>
              <input
                type="email"
            ...
            <div>
              <label className="block text-xs text-slate-500 dark:text-zinc-400 mb-1">Storage tier</label>
              <select
                value={tier}
                onChange={(e) => setTier(e.target.value)}
                className="w-full px-3 py-2 rounded-lg bg-white dark:bg-[#0b111c] border border-slate-300 dark:border-zinc-700 text-sm capitalize"
              >
                {STORAGE_TIERS.map((t) => (
            ...
            {storageError && <p className="text-xs text-red-500 dark:text-red-400">{storageError}</p>}
            {storageMessage && <p className="text-xs text-emerald-600 dark:text-emerald-400">{storageMessage}</p>}
