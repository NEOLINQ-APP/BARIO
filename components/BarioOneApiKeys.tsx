...
export default function BarioOneApiKeys() {
  ...
  async function load() {
    const res = await fetch('/api/bario-one/keys')
  ...
  async function handleRevoke(id: string) {
    await fetch(`/api/bario-one/keys?id=${id}`, { method: 'DELETE' })
    load()
  }

  if (keys === null) return <p className="text-sm text-slate-500 dark:text-zinc-400">Loading…</p>

  return (
    <div className="space-y-6 max-w-2xl">
      <p className="text-sm text-slate-500 dark:text-zinc-400">
        Read/write access to your own Bario One data — customers, invoices, products, sales, employees (read-only). Never used to charge anything or delete data.
      </p>
      ...
      {newRawKey && (
        <div className="rounded-xl border border-amber-500/40 dark:border-[#d4af37]/40 bg-amber-500/5 dark:bg-[#d4af37]/5 p-4 text-sm space-y-2">
          <p className="font-semibold text-amber-700 dark:text-[#d4af37]">Copy this key now — it won&apos;t be shown again.</p>
      ...
      <form onSubmit={handleCreate} className="flex items-center gap-2">
        <input
          ...
          placeholder="Key name (e.g. QuickBooks sync)"
          className="flex-1 px-3 py-2 rounded-lg border border-slate-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm"
        />
        <button type="submit" disabled={creating} className="px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white font-semibold text-sm">
...
{`curl https://www.bario.ca/api/bario-one/v1/customers \\
  -H "Authorization: Bearer bo_flo_live_..."`}
        </pre>
        <p className="text-slate-500 dark:text-zinc-500">
          Also available: <code>/api/bario-one/v1/invoices</code>, <code>/api/bario-one/v1/products</code>, <code>/api/bario-one/v1/sales</code>, <code>/api/bario-one/v1/employees</code>. 60 requests/minute per key.
        </p>
      </div>
    </div>
  )
