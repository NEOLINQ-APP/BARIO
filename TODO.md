...
- [ ] **New reseller mail VPS** (`148.230.94.192`, hostname `reseller.bario.ca`, key `~/.ssh/bario_mail_vps`) — AlmaLinux 9.8 confirmed (cPanel-compatible), 4 vCPU/15GB RAM/197GB disk, port 25 outbound open, PTR being set by Hostinger, DNS A record live. cPanel/WHM install itself hasn't started yet — full mail hosting/reseller setup (license already purchased), waiting on go-ahead to run the installer.
