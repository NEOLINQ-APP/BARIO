    ...
    <main className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased selection:bg-cyan-500 selection:text-slate-950">
      ...
      <section id="plans" className="py-20 px-6 sm:px-12 max-w-6xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
          ...
          <p className="text-slate-600 dark:text-slate-400">Prices in CAD, billed monthly. Cancel or change plans anytime from your billing portal.</p>
        </div>
          ...
          {TIERS.map((t) => (
            <div
              ...
              className={`relative flex flex-col justify-between rounded-2xl p-6 space-y-5 ${
                t.popular
                  ? 'bg-white dark:bg-slate-900 border-2 border-cyan-500 shadow-xl dark:shadow-2xl shadow-cyan-500/10'
            ...
            >
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">{t.label}</h3>
                <div className="text-3xl font-extrabold text-slate-900 dark:text-white">
                  {t.price}
                  {t.price !== '$0' && <span className="text-sm text-slate-600 dark:text-slate-400 font-normal"> /mo CAD</span>}
                </div>
                <div className="text-sm font-semibold text-cyan-600 dark:text-cyan-400">{t.size}</div>
                <p className="text-xs text-slate-500">{t.tagline}</p>
                <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-300 pt-2 border-t border-slate-200 dark:border-slate-800">
                  <li className="flex items-center gap-2"><span className="text-cyan-600 dark:text-cyan-400">✓</span> Photos &amp; videos</li>
                  <li className="flex items-center gap-2"><span className="text-cyan-600 dark:text-cyan-400">✓</span> Folder organization</li>
                  <li className="flex items-center gap-2"><span className="text-cyan-600 dark:text-cyan-400">✓</span> Installable app (PWA)</li>
                  {t.family ? (
                    <li className="flex items-center gap-2"><span className="text-cyan-600 dark:text-cyan-400">✓</span> Family Sharing (5 people)</li>
                  ) : (
                    <li className="flex items-center gap-2 text-slate-500"><span className="text-slate-400 dark:text-slate-600">•</span> Family Sharing on paid plans</li>
                  )}
                </ul>
              </div>
              <a
                ...
                className={`w-full text-center px-4 py-2.5 rounded-xl font-semibold transition-colors ${
                  t.popular
                    ...
                    : 'border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:border-slate-400 dark:hover:border-slate-600'
                }`}
              >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-6">
            <div className="text-center space-y-2">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white">Log in</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400">Or create a free Bario account — no credit card needed to start.</p>
            </div>
            <div className="text-center space-y-2">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white">Open X-Drive</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400">From your Dashboard, or install it to your phone's home screen for one-tap access.</p>
            </div>
            <div className="text-center space-y-2">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white">Upload &amp; upgrade anytime</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400">Start free, pick a paid tier when you need more room — change or cancel from billing anytime.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-6 sm:px-12 max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 p-6 rounded-xl space-y-2">
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">Is this the same as my hosting plan?</h3>
            <p className="text-slate-600 dark:text-slate-400 text-sm">No — storage is billed separately from your site's hosting plan (Free/Starter/Business/Agency). You can be on any hosting plan, or none, and subscribe to storage independently.</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 p-6 rounded-xl space-y-2">
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">How does Family Sharing work?</h3>
            <p className="text-slate-600 dark:text-slate-400 text-sm">On any paid tier, invite up to 4 others by email. Everyone keeps their own login, and the whole family draws from one pooled storage limit — no extra cost.</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 p-6 rounded-xl space-y-2">
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">Can I install it like an app?</h3>
            <p className="text-slate-600 dark:text-slate-400 text-sm">Yes — open X-Drive in your phone's browser and choose "Add to Home Screen." No App Store needed.</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 p-6 rounded-xl space-y-2">
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">What happens if I go over my limit?</h3>
            <p className="text-slate-600 dark:text-slate-400 text-sm">New uploads are blocked until you free up space or upgrade — nothing you've already uploaded is ever deleted.</p>
          </div>
          <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 p-6 rounded-xl space-y-2">
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">Can I cancel anytime?</h3>
            <p className="text-slate-600 dark:text-slate-400 text-sm">Yes, from the billing portal inside X-Drive. Downgrading to Free just pauses new uploads over the free limit — your files stay put.</p>
          </div>
        </div>
      </section>
