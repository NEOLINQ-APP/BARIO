...
export async function POST(req: Request) {
  try {
    ...
    const { customerId, title, stage, valueCents, expectedCloseDate, notes } = await req.json()
    ...
    const id = randomUUID()
    await sql`
      INSERT INTO bo_deals (id, organization_id, customer_id, title, stage, value_cents, expected_close_date, notes, created_by_user_id)
      VALUES (${id}, ${org.id}, ${customerId}, ${title.trim()}, ${dealStage}, ${Number.isFinite(valueCents) ? Math.round(valueCents) : 0}, ${expectedCloseDate || null}, ${notes || null}, ${user.id})
    `
