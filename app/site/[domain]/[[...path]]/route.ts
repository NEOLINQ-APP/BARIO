...
// path. This replaces the old single-segment [domain]/route.ts; the domain
// lookup below is unchanged from that file.
export async function GET(req: Request, { params }: { params: { domain: string; path?: string[] } }) {
  ...
  // Multi-page lookup: only sites with rows in site_pages opt into path-aware
  // rendering. A site with zero rows here (every site created before this
  // feature existed, and any single-page site going forward) renders exactly
  // as before, regardless of what path was requested — no regression risk
  // for existing customer sites.
  ...
  const html =
    pageRows.length > 0 && rawHtml
      ? injectBadgeIntoHtml(injectSeoIntoHtml(rawHtml, seo), showBadge)
